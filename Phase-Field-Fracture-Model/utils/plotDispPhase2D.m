function plotDispPhase2D(PHTelem,tdisp,sizeBasis,numberElements,geometry,controlPts,Mater,istep,miter,StressPlotCheck)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
id_step = num2str(istep);
prev_name= append ('Loadstep_', id_step);
save_name_prev = append(prev_name, '_SaveVariables.mat');

str_save_prev_1 = append(num2str(iter), '_Case_');
...
save_name = append(folder_save,save_name_prev);

if StressPlotCheck==0
    if miter>=1
        LoadSaveVar=load(save_name);
        SaveVar = LoadSaveVar.SaveVar;
    end
    Xx = physcoord(:,1);
    Xdisp = dispcoord(:,1);
    Yy = physcoord(:,2);
    ...
    SaveVar(miter+1).X = Xx;
    SaveVar(miter+1).Y = Yy;
    SaveVar(miter+1).Xdisp = Xdisp;
    SaveVar(miter+1).Ydisp = Ydisp;
    SaveVar(miter+1).Wcoord = wcoord;
end


if StressPlotCheck == 1
    log_SVM = log10(sigmacoord);
    if miter>=1
        LoadSaveVar=load(save_name);
        SaveVar = LoadSaveVar.SaveVar;
    end
    ...
    SaveVar(miter+1).Ydisp = Ydisp;
    SaveVar(miter+1).Wcoord = wcoord;
    SaveVar(miter+1).SVM = sigmacoord; %log_SVM;
end

save(save_name, 'SaveVar')
